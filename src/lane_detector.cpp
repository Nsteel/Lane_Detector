...
ros::Publisher vanishingPoint_pub;
...
void processImage(Preprocessor* preproc, FeatureExtractor<T>* extractor, Fitting<T>* fittingPhase) {
        ...
        std::vector<cv::Point> vanishingPoint = fittingPhase->fitting(copyImg, currentFrame_ptr->image, features);

        std_msgs::Header header; // empty header
        header.stamp = ros::Time::now(); // time
        geometry_msgs::PointStamped vp;
        vp.header = header;
        vp.point.x = vanishingPoint[0].x;
        vp.point.y = vanishingPoint[0].y;
        vanishingPoint_pub.publish(vp);
}
...
int main(int argc, char **argv){
        ...
        vanishingPoint_pub = nh.advertise<geometry_msgs::PointStamped>("lane_detector/vanishing_point", 1);
